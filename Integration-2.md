...
2.
   ...
   * Tokens (Assign, Unassign).
   * Batch Jobs.
   * Authentication Logs.
   * Security Domains (Create, Modify & delete).
   * Token policies (Create, Modify & delete).
   * Lockout policies (Create, Modify & delete).
   * Offline authentication policies (Create, Modify & delete).
   * Password policies (Create, Modify & delete).
   * Self-service troubleshooting policies (Create, Modify & delete).
   * Risk-based authentication policies (Create, Modify & delete).
3. Audit events
4. The following table provides an overview of the types of audit log events that are available in the logs for RSA Authentication Manager with location.
5. Configuration and setting changes
6. In RSA Authentication Manager the log data is usually stored in the internal database. The data can be viewed in different ways. Since, we are using the addon to parse the data in Splunk we should use Rsyslog to transmit the data to Rsyslog Server. First we will configure RSA to send data to Rsyslog Server as follows
7.
   1. Go to Operations Console and navigate to Administration > Operating System Access and enable SSH.
   2. Navigate to Setup -> logging -> select your instance Select Save to internal database and local operating system syslog. Now move to SSH and execute the following command
   3. Change the directory to /opt/rsa/am/server.
   4. Run ./rsaserv restart all.
8.

    ![image](https://user-images.githubusercontent.com/80450749/216576022-d1d3173f-5393-45e9-a1b2-355c2633595b.png) ![image](https://user-images.githubusercontent.com/80450749/216576183-20b02a11-d57d-43af-90dd-67ede846654d.png) ![image](https://user-images.githubusercontent.com/80450749/216576244-8d951dd9-2ff6-4875-9915-e40312f45a03.png)
9.
...
11.
    ...
    8. Now restart your rsyslog using systemctl restart rsyslog.
12. ```
    ########################################Admin Logs#################################
...
14.
    ...
    4. In the backend navigate to /opt/splunk/etc/apps/Splunk\_TA\_rsa-securid/local.
...
20.
    ...
    13. Create outputs.conf in /opt/splunk/etc/system/local/ location and add the following lines:
21. ```
...
22.
    ...
    15. Save and exit the file and restart Splunk using /opt/splunk/bin/splunk restart.
23. Reference
24.
    * Audit Log Configuration: https://community.rsa.com/t5/securid-authentication-manager/configure-logging/ta-p/629844
    * List of Actions that can be performed by user: https://community.rsa.com/t5/securid-authentication-manager/rsa-authentication-manager-log-messages/ta-p/630160
    * Splunk Add-on for RSA SecurID: https://docs.splunk.com/Documentation/AddOns/released/RSASecurID/About
